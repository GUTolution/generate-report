...
#let (show_gleneagles_logo: show-gleneagles-logo) = sys.inputs.at("args", default: (show_gleneagles_logo: true))
...
#let longest-non-alpha-non-numeric-sequence(s) = {
  s.split(" ").filter(substr => (not is-alpha(substr)) and (not is-numeric(substr))).join(" ")
}
      ...
      align(horizon, footer-text(table(
        ...
        pad(x: 15pt, {
          ...
          if heading-components != none {
            ...
            // if heading
            longest-non-alpha-non-numeric-sequence(to-string(heading-components))
          }
        }),
        align(
          left,
